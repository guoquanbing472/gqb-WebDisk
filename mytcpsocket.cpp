...
void MyTcpSocket::copyDir(QString strSrcDir, QString strDestDir)
{
    ...
    dir.setPath(strSrcDir);//设置当前目录
    QFileInfoList fileInfoList = dir.entryInfoList();//获取源目录下所有文件和子目录的信息
    QString srcTmp;
    QString destTmp;
    for(int i = 0; i < fileInfoList.size(); i++)
    {
        ...
        {
            srcTmp += strSrcDir + '/' + fileInfoList[i].fileName();//拼接源路径和目标路径，生成完整源文件路径
            destTmp = strDestDir + '/' + fileInfoList[i].fileName();//生成完整的目标文件路径
            QFile::copy(srcTmp, destTmp);//将该文件复制到目标路径
        }
...
void MyTcpSocket::recvMsg()
{
    if(!m_bUpload)
    {
        ...
        unit uiPDULen = 0;
        this->read((char*)&uiPDULen, sizeof(unit));//从套接字中读取数据
        unit uiMsgLen = uiPDULen - sizeof(PDU); //消息的有效载荷长度
        PDU *pdu = mkPDU(uiMsgLen);//根据消息的长度 uiMsgLen 创建一个 PDU 结构体指针 pdu
        this->read((char*)pdu + sizeof(unit), uiPDULen - sizeof(unit));//从套接字读取剩余 uiPDULen - sizeof(unit) 字节的数据，填充到 pdu 指针指向的内存区域中
        ...
        case ENUM_MSG_TYPE_REGIST_REQUEST:
        {
            ...
            write((char*)respdu, respdu->uiPDULen);//向客户端发送数据
            free(respdu);
            respdu = NULL;
            break;
        ...
        case ENUM_MSG_TYPE_FLUSH_FRIEND_REQUEST:
        {
            ...
            for(int i = 0; i * 2 < strList.size(); i++)//2*i:好友名，2*i+1：好友在线信息
            {
                memcpy((char*)(respdu->caMsg) + 36 * i, strList.at(i * 2).toStdString().c_str(), 32);
                memcpy((char*)(respdu->caMsg) + 36 * i + 32, strList.at(i * 2 + 1).toStdString().c_str(), 4);//复制每段36字节，32好友名4在线状态
...
{
    m_pTimer->stop();
    char *buffer = new char[4096];//为了避免内存占用过多，使用4kb缓冲区
    qint64 ret = 0;
    while(true)
    {
